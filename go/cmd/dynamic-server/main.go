...
func main() {
	...
	registry := bsr.NewRegistry(client)
	...
	for {
						...
						Tools: []*mcp.Tool{
							{
								...
								Description: "Dynamically resolved BSR tool (GetModuleRequest)",
		...
		case *mcp.MCPMessage_CallToolRequest:
			...
			_, err := registry.Resolve(context.Background(), toolRef)
			if err != nil {
				log.Fatalf("Server failed to resolve schema: %v", err)
			}

			args, err := registry.Unpack(payload.CallToolRequest.Arguments)
			if err != nil {
